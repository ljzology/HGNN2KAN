...
import dhg
import torch
# 用dhg.random.hypergraph_Gnm()创建一个超图，节点数为num_v，节点特征维度fea_dim=16
def create_hypergraph(num_v, fea_dim=16, num_classes=8):
    # 创建超图
    hypergraph = dict()
    hypergraph["num_vertices"] = num_v
    hypergraph["num_edges"] = num_v // 8
    hypergraph["num_classes"] = num_classes

    # 生成超边，一条超边可能有多个节点，用元组来表示超边，把所有超边放在一个列表里
    hypergraph_tmp = dhg.random.hypergraph_Gnm(num_v, num_v // 8)
    hypergraph["edge_list"] = hypergraph_tmp.e[0]

    # 生成节点特征，使用torch生成
    features = torch.rand(num_v, fea_dim)
    hypergraph["features"] = features
    
    # 生成标签
    labels = torch.randint(0, num_classes, (num_v,))
    hypergraph["labels"] = labels
...
def load_data(name):
    if name == 'cora':
        ...
        num_v = int(name.split('_')[1])
        data = create_hypergraph(num_v)
        edge_list = data['edge_list']
    else:
